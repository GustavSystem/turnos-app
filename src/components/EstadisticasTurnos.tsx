    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-[95%] max-w-6xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-lg sm:text-xl font-bold">Estadísticas de Turnos {año}</h2>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-50 p-3 rounded-lg">
                <h3 className="text-md font-semibold text-blue-800 mb-1">Horas Totales del Año</h3>
                <p className="text-xl font-bold text-blue-600">{horasTotalesAnio} horas</p>
              </div>
              <div className="bg-green-50 p-3 rounded-lg">
                <h3 className="text-md font-semibold text-green-800 mb-1">Horas Reales Esperadas</h3>
                <input
                  type="number"
                  value={estadisticas.horasRealesEsperadas}
                  onChange={handleHorasRealesChange}
                  className="w-full p-1 border rounded focus:ring-2 focus:ring-green-500 focus:border-transparent"
                />
              </div>
              <div className="bg-purple-50 p-3 rounded-lg">
                <h3 className="text-md font-semibold text-purple-800 mb-1">Diferencia</h3>
            ...
            <div>
              <h3 className="text-lg font-semibold mb-2">Horas por Mes</h3>
              <div className="space-y-2">
                {meses.map((mes, index) => (
                  <div key={mes} className="border rounded-lg p-3">
                    <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                      <h4 className="font-semibold">{mes}</h4>
                      <p className="text-lg font-bold text-blue-600">
                        {estadisticas.horasPorMes[index] || 0} horas
                      </p>
                      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-sm text-gray-600">
                        {turnosPorMes[index]?.map(turno => (
                          <span key={turno.letra} className="whitespace-nowrap">
                            <span className="font-medium text-gray-800">T.{turno.letra}:</span>
                            &nbsp;{turno.count}d ({turno.horas * turno.count}h)
                          </span>
                        ))}
                      </div>
                    </div>
