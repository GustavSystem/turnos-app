    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
      <div className="absolute inset-0 bg-white flex flex-col">
        ...
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-2xl font-bold text-gray-800">Estadísticas de Turnos {año}</h2>
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700 text-3xl leading-none p-2"
            aria-label="Cerrar"
          >
            &times;
          </button>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-blue-50 p-4 rounded-lg shadow-sm text-center">
                <h3 className="text-lg font-semibold text-blue-800 mb-2 uppercase">Horas Totales Año</h3>
                <p className="text-4xl font-bold text-blue-600">{horasTotalesAnio}h</p>
              </div>
              <div className="bg-green-50 p-4 rounded-lg shadow-sm text-center">
                <h3 className="text-lg font-semibold text-green-800 mb-2 uppercase">Horas Reales Esperadas</h3>
                <input
                  type="number"
                  value={estadisticas.horasRealesEsperadas}
                  onChange={handleHorasRealesChange}
                  className="w-full p-4 border border-gray-300 rounded text-center text-2xl focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-700"
                />
              </div>
              <div className="bg-purple-50 p-4 rounded-lg shadow-sm text-center">
                <h3 className="text-lg font-semibold text-purple-800 mb-2 uppercase">Diferencia</h3>
            ...
            {/* Monthly Stats */}
            <div>
              <h3 className="text-2xl font-semibold mb-4 text-gray-800">Horas por Mes</h3>
              <div className="space-y-4">
                {meses.map((mes, index) => (
                  <div key={mes} className="pb-4 border-b border-gray-200 last:border-b-0">
                    <div className="flex flex-col gap-3">
                      <h4 className="font-semibold text-blue-700 text-xl">{mes}: {horasPorMes[index] || 0}h</h4>
                      <div className="flex flex-wrap gap-2">
                        <span className="font-medium text-lg text-gray-700">Turnos:</span>
                        {turnosPorMes[index]?.map(turno => (
                          <div key={turno.letra} className="flex items-center bg-gray-100 px-4 py-3 rounded">
                            <span className="font-medium text-lg text-gray-700">{turno.letra}:</span>
        ...
        <div className="p-4 border-t border-gray-200">
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-8 rounded-lg transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 text-xl"
          >
            Guardar
          </button>
        </div>
